...
public class WorkbenchBlockEntityRenderer implements BlockEntityRenderer<VisualCraftingTableBlockEntity> {
    ...
    @Override
    public void render(VisualCraftingTableBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        for (int i = 0; i < blockEntity.getCraftSlots().getContainerSize(); ++i) {
            ItemStack itemStack = blockEntity.getCraftSlots().getItem(i);
            if (!itemStack.isEmpty()) {
                this.renderIngredientItem(blockEntity, partialTicks, poseStack, bufferIn, combinedLightIn, combinedOverlayIn, i, itemStack);
            }
        }
        if (VisualWorkbench.CONFIG.get(ClientConfig.class).renderResult && blockEntity.getResultSlots().getRecipeUsed() != null) {
            ItemStack resultItem = blockEntity.getResultSlots().getRecipeUsed().getResultItem(Minecraft.getInstance().level.registryAccess());
            if (!resultItem.isEmpty()) {
                this.renderResultItem(resultItem, blockEntity.getLevel(), blockEntity.ticks + partialTicks, poseStack, bufferIn, combinedLightIn);
            }
        }
    }

    private void renderIngredientItem(VisualCraftingTableBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, int i, ItemStack itemStack) {
        poseStack.pushPose();
    ...
    private void setupFloatingRenderer(VisualCraftingTableBlockEntity blockEntity, float partialTicks, PoseStack poseStack, ItemStack itemStack, int index) {
        // -0.0125 to 0.0125
    ...
    private void setupLayingRenderer(VisualCraftingTableBlockEntity blockEntity, float partialTicks, PoseStack poseStack, ItemStack itemStack, int index) {
        BakedModel model = this.itemRenderer.getModel(itemStack, null, null, 0);
        boolean blockItem = model.isGui3d();
